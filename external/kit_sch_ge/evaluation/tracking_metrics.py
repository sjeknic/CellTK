...
def calc_tra_score(result_dir, gt_dir, get_aogm_results=True):
    ...
    tra_measure_file = [file
                        for file in os.listdir(Path(tra_measure_dir, platform_name))
                        if file.startswith('TRAMeasure')]
    assert len(tra_measure_file) == 1, 'just one tracking measure executable per sub directory allowed'
    ...
    n_files = len([f
                   for f in os.listdir(data_dir/data_set_id)
                   if f.endswith('tif')])
    if n_files < 1000:
        n_digits = 3
    else:
    ...
    output = subprocess.Popen([tra_measure_path.as_posix(),
                               data_dir.as_posix(), data_set_id, str(n_digits)],
...
def calc_ctc_scores(result_dir, gt_dir):
    ...
    for measure_f in measure_files:
        if not os.path.exists(default_res_dir / 'res_track.txt') and measure_f.startswith('TRA'):
            continue
        ctc_metric_path = Path(ctc_measure_dir, platform_name, measure_f)
        output = subprocess.Popen([ctc_metric_path.as_posix(), data_dir.as_posix(),
                                   data_set_id, str(n_digits)],
                                  stdout=subprocess.PIPE)
        result, _ = output.communicate()
    ...
    all_results = {metric_name: float(score[0]) for metric_name, score in all_results.items()}
    return all_results
...
def extract_aogm_info(tra_log_file):
    ...
    with open(tra_log_file, "r") as f:
        ...
        errors_over_t = {'EdgeSplit': {},  # time id: value
                         'EdgeAdd': {},
                         'EdgeDelete': {},
                         'EdgeSemantic': {},
                         'FP': {},
